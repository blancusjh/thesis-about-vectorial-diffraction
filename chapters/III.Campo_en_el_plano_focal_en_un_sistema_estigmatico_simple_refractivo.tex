...
\eqref{eq:E_prime_func_of_M_E0} debe distinguirse de la fórmula canónica del
campo focal derivada por Richards y Wolf~\cite{RichardsWolf1959} (véase
también \cite{Torok1995}). En esta última, el sistema se modela como una lente
aplanática ideal que satisface la condición de los senos de
Abbe~\cite{Abbe1873}, y la amplitud sobre la pupila queda ponderada por el
factor apodizador $\sqrt{\cos\theta}$. En la formulación presente, la función
pupila vectorial
$\mathbb E^{\mathscr G'}=\mathbb T\mathbb E_0/\ell_0$ se construye punto a
punto a partir de la geometría de la superficie cartesiana y de la
transmisión de Fresnel evaluada localmente. La factorización $1/\ell_0$
incorpora la divergencia esférica sobre $\mathscr G$, mientras que las
variaciones angulares de $t_p$ y $t_s$ producen la modulación de amplitud y el
acoplamiento vectorial. De esta manera, la transformación vectorial se
obtiene directamente de la superficie refractora considerada.
...
a la fase global $\gamma$. Después de la transmisión, el estado sobre $\mathscr{G}'$ queda caracterizado por
...
\end{equation}
Esta expresión muestra que la transmisión vectorial puede ser representada 
mediante una transformación de Möbius sobre el estado local de polarización, la cual es invertible siempre
...
La ecuación \eqref{eq:zeta_focal_mobius} determina el estado de polarización 
en el plano focal, producto de la refracción del campo incidente con amplitudes 
$(\mathscr E_x, \mathscr E_y)$.

Nótese la diferencia esencial con la transformación local de la
sección~\ref{subsec:polarizacion_refraccion}: las amplitudes $A_{x,y}$ y
las fases $\Phi_{x,y}$, definidas en \eqref{eq:Axy_Phixy_def}, en un punto de observación son funcionales de la distribución completa del campo sobre la pupila. En general no
existe una relación local la polarización del campo incidente $\zeta(\rho',\phi')$ y la polarización del campo transmitido en el plano focal $\zeta_f(\vb r)$.

Vale la pena destacar, que como se puede observar desde la 
ecuación \eqref{eq:theta_delta_focal}, la propagación del 
campo añade una fase generalmente distinta a cada componente, $\Phi_x$ y $\Phi_y$. La
...
polarizada a difrencia de la transformación del campo incidente al campo en la pupila 
que no introducia elipticidad si la iluminación incidente era polarizada linealmente. 
\\
Sin embargo, si la iluminación incidente es simetrica axialmente y las componentes 
del vector de amplitud $\mathbb E_0$ son reales, $\Phi_x=\Phi_y=-$ por las propiedades de 
la transformada de Fourier para funciones reales y pares \cite{arfken2013}.
